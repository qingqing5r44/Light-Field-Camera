...
{
public:
    MeshTriangle(const std::string& filename, float x, float y, float z, float scale, MaterialType mt)
    {
        objl::Loader loader;
        ...
        for (int i = 0; i < mesh.Vertices.size(); i += 3) {
            ...
            for (int j = 0; j < 3; j++) {
                auto vert = Vector3f(mesh.Vertices[i + j].Position.X,
                                     ...
                                     mesh.Vertices[i + j].Position.Z) * scale + Vector3f(x, y, z);
...
Vector3f edge1 = v1 - v0;
    Vector3f edge2 = v2 - v0;
    Vector3f pvec = crossProduct(ray.direction, edge2);
    float det = dotProduct(edge1, pvec);
    if (det == 0 || det < 0)
        return inter;
    ...
    // // rayTriangleIntersect(v0, v1, v2, ray.origin,ray.direction, uP, vP, tP);
    // // u = uP;
    // // v = vP;
    // // t = tP;
    // // end my code
    ...
    // Vector3f e1, e2, tVec, pVec, qVec;
    // float det, inv_det;
    // e1 = v1 - v0;
    // e2 = v2 - v0;
    // pVec = crossProduct(ray.direction, e2);
    // det = dotProduct(e1, pVec); // if det is near zero, ray lies in plane of triangle
